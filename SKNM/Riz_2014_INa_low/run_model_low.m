...
current_matrix = zeros(length(T_high), 14);
ylabels_sub = {'mV', 'INa high', 'INa low', 'IBK', 'ICaL', 'ICaPQ', 'ICaT', 'IGABAR', 'IHERG', 'IKATP', 'IKv', 'ISK', 'Ileak', 'Istim', 'Ivclamp'};

for i = 1: length(T_high)
    [~, currents2] = Riz2014_rhs_INa_low(T_high(i), Y_high(i,:), param);
...
subplot(8,2,1), 
plot(T_high(index_plot)/1000, Y_high(index_plot,15), 'linewidth', 1)
set(gca, 'fontsize', 14); xlabel('t (sec)'); ylabel(ylabels_sub{1});
