function [values, currents] = Riz2014_rhs_INa_low(t, states, parameters, varargin)
  % Compute the right hand side of the Riz2014 ODE

  % Assign states
  if length(states)~=15
    error('Expected the states array to be of size 15.');
  end
  ...
  % Assign parameters
  if length(parameters)~=86
    error('Expected the parameters array to be of size 86.');
  end
