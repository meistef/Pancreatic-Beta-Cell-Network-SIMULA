function [] = process_vclamp(directory)
    ...
    disp(strcat('No. of failed exocytosis analyses: ', num2str(sum(isnan(data(:,5))))));
    disp(strcat('No. of failed peak I_K analyses: ', num2str(sum(isnan(data(:,8))))));
    disp(strcat('No. of failed fraction A-type I_K analyses: ', num2str(sum(isnan(data(:,9))))));
    disp(strcat('No. of failed I_{K,tail} analyses: ', num2str(sum(isnan(data(:,10))))));
    ...
    subplot(2,1,2), histogram('BinEdges', vhalf_edges, 'BinCounts', vhalf_counts), hold on, plot(vhalf_edges,vhalf_INa_fit,'r','LineWidth',2), ylabel('Density'), xlabel('I_{Na} V_{1/2} (mV)')
    ...
    figure
    subplot(2,2,1), histogram('BinEdges', EE_edges, 'BinCounts', EE_counts), hold on, plot(EE_edges,EE_fit,'r','LineWidth',2), ylabel('Density'), xlabel('Early exocytosis (via Ca^{2+}_i, \muM)')
    subplot(2,2,2), histogram('BinEdges', LE_edges, 'BinCounts', LE_counts), hold on, plot(LE_edges,LE_fit,'r','LineWidth',2), ylabel('Density'), xlabel('Late exocytosis (via Ca^{2+}_i, \muM)')
    subplot(2,2,3), histogram('BinEdges', TE_edges, 'BinCounts', TE_counts), hold on, plot(TE_edges,TE_fit,'r','LineWidth',2), ylabel('Density'), xlabel('Total exocytosis (via Ca^{2+}_i, \muM)')
